module.exports = function init(site) {
  ...
  site.post({
    name: '/api/orders/upload/cardImage/orders',
    require: {
      permissions: []
    }
  }, (req, res) => {
  ...
  site.post('/api/orders/updateToStatusShipped', (req, res) => {
   ...
   // update To Status Hidden
   site.post('/api/orders/updateToStatusHidden', (req, res) => {
    req.headers.language = req.headers.language || 'en'
    let response = {}
    ...
    $orders.findOne({
      ...
      if (doc) {
        ...
        if (doc.holdingArr&&doc.holdingArr.length > 0) {
          let arr = []
          arr.push(req.body.pharmacyId)
          doc.holdingArr.push.apply(doc.holdingArr, arr)
        }
        if (doc.holdingArr && doc.holdingArr.length == 0) {
          doc.holdingArr.push(req.body.pharmacyId)
        }
      
        $orders.update(doc, (err, result) => {
          response.done = true, 
          response.errorCode = site.var('succeed');
          response.message = site.word('updatedSuccessfully')[req.headers.language];
          res.json(response);
